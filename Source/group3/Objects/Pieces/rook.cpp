...
Rook::Rook(float r, float g, float b)
    : m_r(r), m_g(g), m_b(b)
...
unsigned int Rook::compileShader(unsigned int type, const char* src)
{
...
void Rook::draw(float x, float y)
{
...
void Rook::addRect(std::vector<float>& v, float x0, float y0, float x1, float y1)
{
    v.insert(v.end(), {
        x0, y0,  x1, y0,  x1, y1,
        x0, y0,  x1, y1,  x0, y1
...
void Rook::addTrapezoid(std::vector<float>& v,
...
void Rook::addTriangle(std::vector<float>& v,
    float x0, float y0,
    float x1, float y1,
    float x2, float y2)
{
...
void Rook::addCircle(std::vector<float>& v, float cx, float cy, float radius, int segments)
{
...
std::vector<float> Rook::buildVertices()
{
    std::vector<float> v;
    ...
    // body
    addRect(v, -0.045f, -0.110f, 0.045f, 0.025f);

    // Head
    addRect(v, -0.07f, 0.025f, 0.07f, 0.07f);

    // battlements
    addRect(v, -0.07f, 0.07f, -0.04f, 0.09f);
    addRect(v, -0.02f, 0.07f, 0.02f, 0.09f);
    addRect(v, 0.04f, 0.07f, 0.07f, 0.09f);
